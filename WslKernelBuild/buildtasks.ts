...
export class buildTasks {
  ...
  public readFromFile = () => {
    let obj = JSON.parse(fs.readFileSync(__dirname + path.sep + "tasks.json", "utf-8"));
    this.tasks = <buildTask[]>obj.tasks;
    this.current = <number>obj.current;
    if(this.tasks[this.current].status==='pending' || this.tasks[this.current].status==='building'){
      this.current-=1;
      this.next();
    }
  };
  public saveToFile = () => {
    ...
    fs.writeFileSync(__dirname + path.sep +"tasks.json", JSON.stringify(obj), "utf-8");
  };
  public next = () => {
    ...
    if (!this.buildConfig.check(this.tasks[this.current].config)) {
      ...
      startBuildTask(this.tasks[this.current].link, this.tasks[this.current].config).then(() => {
        this.tasks[this.current].status = "done";
        this.saveToFile();
        this.next();
      });
  ...
  public newTask(name: string, link: string, config: string) {
    ...
    if (this.tasks.length <= 1 || this.tasks[this.tasks.length -2].status === "done") 
      this.next();
    this.saveToFile();
    return;
  }
